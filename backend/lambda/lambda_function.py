...
EMBEDDING_MODEL = "text-embedding-3-large"  # all DB data is 3072-dim
EMBEDDING_DIM = 3072
SUBVEC_DIM = int(os.environ.get("SUBVEC_DIM", "512"))  # truncated dim for fast sequential scan
...
def vector_search(
    ...
    halfvec キャストで HNSW インデックスを利用 (>2000 dims 対応)
    """
    conn = get_db_connection()
    d = SUBVEC_DIM  # use truncated dims for faster scan (subvec reduces 3072→d)
    try:
...
【パターン4: 不明確・意味不明な入力】
例: 断片的な言葉、文法的に不完全な文、意味が取れないメッセージ

→ 回答形式:
1. 温かく受け入れる
2. 入力の意図を推測して真の父母様のみ言葉に関連付ける
3. もし全く関連が見つからない場合でも、励ましのメッセージと「どのようなことを知りたいですか？」と優しく問いかける

========================================
注意事項
========================================

- どんな入力に対しても必ず何らかの回答を返す。エラーメッセージや「分かりません」だけで終わらない
- 不明確な入力は、最も近い神学的テーマに関連付けて回答する
- 引用元が不明な場合は「見つかりませんでした」と伝えつつ、関連するテーマについて案内する
...
def generate_chat_response(
    ...
    _newer_models = CHAT_MODEL.startswith("gpt-5") or CHAT_MODEL in ("o1", "o1-mini", "o3", "o3-mini")
    create_kwargs = dict(model=CHAT_MODEL, messages=messages)
    if not _newer_models:
        create_kwargs["temperature"] = 0.8
        create_kwargs["max_tokens"] = 700
    else:
        create_kwargs["max_completion_tokens"] = 700
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Internal: DB info / diagnostics
        if message == "__db_info__":
            conn = get_db_connection()
            cur = conn.cursor()
            try:
                cur.execute("SELECT extversion FROM pg_extension WHERE extname='vector'")
                pgvec = (cur.fetchone() or ["?"])[0]
                ...
                cur.execute("SELECT column_name, udt_name, character_maximum_length FROM information_schema.columns WHERE table_name='documents' AND column_name='embedding'")
                emb_col = cur.fetchone()
                return {"statusCode": 200, "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"}, "body": json.dumps({"pgvector": pgvec, "rows": n, "indexes": indexes, "embedding_col": list(emb_col) if emb_col else None})}
        ...
        # Internal: create pgvector HNSW index for fast similarity search
        if message == "__create_index__":
            conn = get_db_connection()
            conn.autocommit = True
            cur = conn.cursor()
            try:
                cur.execute("SET statement_timeout = '0'")  # index creation needs unlimited time
                cur.execute("SELECT extversion FROM pg_extension WHERE extname='vector'")
                pgvec = (cur.fetchone() or ["?"])[0]
                cur.execute("SELECT indexdef FROM pg_indexes WHERE tablename='documents' AND indexname='documents_embedding_hnsw'")
                row = cur.fetchone()
                if row:
                    return {"statusCode": 200, "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "hnsw exists", "indexdef": row[0]})}
                cur.execute("SELECT COUNT(*) FROM documents")
                n = cur.fetchone()[0]
                d = SUBVEC_DIM
                print(f"[Index] pgvector={pgvec} rows={n} subvec_dim={d}. Trying ALTER TABLE then subvector HNSW...")
                # Step 1: give column explicit dims so subvector() works in index expressions
                try:
                    cur.execute(f"ALTER TABLE documents ALTER COLUMN embedding TYPE vector({EMBEDDING_DIM}) USING embedding::vector({EMBEDDING_DIM})")
                    print(f"[Index] ALTER TABLE embedding→vector({EMBEDDING_DIM}) done")
                except Exception as ae:
                    print(f"[Index] ALTER TABLE skipped: {ae}")
                # Step 2: create HNSW on subvector expression (d < 2000 limit)
                try:
                    cur.execute(f"""
                        CREATE INDEX CONCURRENTLY IF NOT EXISTS documents_embedding_hnsw
                        ON documents USING hnsw ((subvector(embedding,1,{d})::vector({d})) vector_cosine_ops)
                        WITH (m=16, ef_construction=64)
                    """)
                    print(f"[Index] HNSW created on subvector({d})")
                    return {"statusCode": 200, "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "hnsw created", "dim": d, "rows": n, "pgvector": pgvec})}
                except Exception as ie:
                    print(f"[Index] HNSW create failed: {ie}")
                    return {"statusCode": 200, "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"}, "body": json.dumps({"status": "no_index", "error": str(ie), "rows": n})}
            except Exception as e:
        ...
        print(f"[Timing] Embed: {_t.time()-_t0:.1f}s")
